function result = main_script(part_idx, disp_idx)
close all
result = struct();
%% save to a diary?????
...
switch part_idx
    ...
    case 2
        ...
        display(sprintf('Image%d: average grid point reprojection error using original and new H',imageidx(i)));
        display([mean(error_old),mean(error_new)]);
        %
...
% 2.2 projecting grid corners to the image by H and calculate the reprojection
...
for i = 1:4
    ...
    error_rep = sum((p_correct(:,1:2,i)-p_projection(1:2,:,i)').^2,2);
end
end
